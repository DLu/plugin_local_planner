...
bool LatchedStopRotateController::isPositionReached(LocalPlannerUtil* planner_util,
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
      plugin_local_planner::getGoalPositionDistance(global_pose, goal_x, goal_y) <= xy_goal_tolerance) {
...
bool LatchedStopRotateController::isGoalReached(LocalPlannerUtil* planner_util,
  ...
  plugin_local_planner::LocalPlannerLimits limits = planner_util->getCurrentLimits();
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
    ...
    double angle = plugin_local_planner::getGoalOrientationAngleDifference(global_pose, goal_th);
    //check to see if the goal orientation has been reached
    if (fabs(angle) <= limits.yaw_goal_tolerance) {
      //make sure that we're actually stopped before returning success
      if (plugin_local_planner::stopped(base_odom, rot_stopped_vel, trans_stopped_vel)) {
        return true;
      }
    }
  }
...
bool LatchedStopRotateController::rotateToGoal(
  ...
  if (valid_cmd) {
    ROS_DEBUG_NAMED("plugin_local_planner", "Moving to desired goal orientation, th cmd: %.2f, valid_cmd: %d", v_theta_samp, valid_cmd);
...
bool LatchedStopRotateController::computeVelocityCommandsStopRotate(geometry_msgs::Twist& cmd_vel,
  ...
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
    ...
    if ( ! rotating_to_goal_ && !plugin_local_planner::stopped(base_odom, limits.rot_stopped_vel, limits.trans_stopped_vel)) {
